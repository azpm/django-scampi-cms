...
class PublishedStoryFeed(Feed):
    ...
    def item_link(self, item):
        return reverse('cms:publish:pubstory-detail',
                       kwargs={'picker': self.picker.keyname, 'year': item.start.year, 'month': item.start.month,
                               'day': item.start.day, 'slug': item.slug})
