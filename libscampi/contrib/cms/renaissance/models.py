...
from libscampi.contrib.cms.renaissance import validation
...
class Media(models.Model):
    ...
    def save(self, *args, **kwargs):
        if hasattr(self, 'file') and getattr(self, 'file', None) and not self.mime_type:
            f = getattr(self, 'file')
            self.mime_type = mimetypes.guess_type(f.name)[0]
...
class Image(Media):
    file = models.ImageField(upload_to=_file_upload_pathing, validators=[validation.ValidImgExtension()])
...
class Video(Media):
    file = models.FileField(upload_to=_file_upload_pathing, validators=[validation.ValidVidExtension()], blank=True)
...
class Audio(Media):
    file = models.FileField(upload_to=_file_upload_pathing, validators=[validation.ValidAudExtension()])
...
class Document(Media):
    file = models.FileField(upload_to=_file_upload_pathing, validators=[validation.ValidDocExtension()])
...
class Object(Media):
    file = models.FileField(upload_to=_file_upload_pathing, validators=[validation.ValidObjExtension()])
