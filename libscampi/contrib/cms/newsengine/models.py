...
class ArticleTranslation(models.Model):
    ...
    headline = models.CharField(_('Article Headline'), max_length=255, help_text=_("Article Title. No markup allowed."))
    sub_headline = models.CharField(_('Article Tag line'), max_length=255, help_text=_("Will be truncated to 30 words when viewed as a spotlight.  No markup allowed."))
    synopsis = models.TextField(blank=True, help_text=_("Article Synopsis, markup(down) allowed: see <a href='http://daringfireball.net/projects/markdown/syntax'>Markdown Syntax</a> for help"))
    body = models.TextField(blank=True, help_text=_("Article body, markup(down) allowed: see <a href='http://daringfireball.net/projects/markdown/syntax'>Markdown Syntax</a> for help"), validators=[validate_article])
...
class Article(MultilingualModel):
    ...
    image_inlines = models.ManyToManyField('renaissance.Image', blank=True, verbose_name=_("Images"), help_text="Images (inline)")
    video_inlines = models.ManyToManyField('renaissance.Video', blank=True, verbose_name=_("Videos"), help_text="Videos (inline)")
    audio_inlines = models.ManyToManyField('renaissance.Audio', blank=True, verbose_name=_("Audio"), help_text="Audios (inline)")
    document_inlines = models.ManyToManyField('renaissance.Document', blank=True, verbose_name=_("Documents"), help_text="Documents (inline)")
    object_inlines = models.ManyToManyField('renaissance.Object', blank=True, verbose_name=_("HTML Objects"), help_text="Objects (inline)")
    external_inlines = models.ManyToManyField('renaissance.External', blank=True, verbose_name=_("Widgets"), help_text="Externals (inline)")
...
class Story(models.Model):
    ...
    def related(self):
        ...
        ).exclude(Q(pk=self.pk)).annotate(rel_count=Count('categories')).order_by('-rel_count', 'important').values(
            'rel_count', 'id', 'slug', 'article')[:10]
