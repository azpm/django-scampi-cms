...
class PublishedManager(models.Manager):
    ...
    def find_related(self, story):
        ...
        return qs\
            .order_by('-rel_count')\
            .values('rel_count', 'story', 'story__slug', 'story__article', 'start', 'slug')\
            .distinct()


class CategoryGenera(models.Manager):
    def for_cloud(self, qs):
        queryset = super(CategoryGenera, self).get_query_set().distinct()
        return queryset\
            .filter(story__publish__in=qs, browsable=True)\
            .annotate(occurances=Count('story'))\
            .values('id', 'title', 'keyname', 'occurances')
