...
from libscampi.contrib.cms.communism import managers, utils

__all__ = ['Theme', 'StyleSheet', 'Javascript', 'Realm', 'RealmNotification', 'Section', 'MagicSection', 'Commune',
           'Slice', 'NamedBoxTemplate', 'NamedBox', 'Application']
...
class Theme(models.Model):
    ...
    banner = models.ImageField(upload_to=utils.theme_banner_decorator, verbose_name=_("Image Banner"))
...
class HtmlLinkRef(models.Model):
    ...
    base_active = managers.BaseActiveLinkRefs()
...
class Javascript(HtmlLinkRef):
    ...
    file = models.FileField(upload_to=utils.theme_script_decorator, null=True, blank=True, storage=OverwriteStorage())
...
class StyleSheet(HtmlLinkRef):
    ...
    file = models.FileField(upload_to=utils.theme_style_decorator, storage=OverwriteStorage())
...
class Realm(models.Model):
    ...
    search_collection = models.CharField(max_length=200, null=True, blank=True,
                                         help_text=_("Keyname for search collections"))
    direct_link = models.BooleanField(default=False, verbose_name=_(
        "Provides a direct link to some external site outside of your Scampi install"))
    theme = models.ForeignKey(Theme, help_text=_("Provides Fall Back theme for sections/applications"))
    objects = managers.RealmManager()
    ...
    def primary_section(self):
        ...
        try:
            return Section.objects.filter(active=True, extends=None, realm__id=self.id).order_by('display_order')[0]
        except IndexError:
...
class Section(models.Model):
    ...
    localised = managers.LocalizedSectionManager()
    ...
    def breadcrumb_helper(self):
        ...
        return "{0:>s}".format(utils.section_path_up([self], "."))
    ...
    def get_real_absolute_url(self):
        ...
        if realm.secure:
            return "https://{0:>s}/{1:>s}/".format(realm.site.domain, utils.section_path_up([self], "."))
        else:
            return "http://{0:>s}/{1:>s}/".format(realm.site.domain, utils.section_path_up([self], "."))
...
class BaseHierarchyElement(models.Model):
    section = GenericRelation(Section, content_type_field="element_type", object_id_field="element_id")
    name = models.CharField(_("Display Name"), max_length=100)
    description = models.TextField(null=True, blank=True)

    localised = managers.LocalizedElementManager()
    ...
    def get_absolute_url(self):
        return "/{0:>s}/".format(utils.section_path_up([self.container], "."))
...
class Commune(BaseHierarchyElement):
    ...
    objects = managers.CommuneManager()
...
class NamedBox(models.Model):
    ...
    active = models.BooleanField(default=True, db_index=True)
    # TODO this is a data model issue for Conduit and Communism to depend on each other
    content = models.ForeignKey("conduit.DynamicPicker", null=True, blank=True, on_delete=models.SET_NULL)

    objects = managers.NamedBoxManager()
    ...
    def natural_key(self):
        return (
            self.slice.commune.keyname,
            self.slice.display_order,
            self.gridy,
            self.gridx,
            self.display_order,
            self.keyname
    ...
    def picker(self):
        if self.content:
            return self.content
        elif hasattr(self, 'staticpicker'):
...
models.signals.post_save.connect(utils.cache_namedbox_template, sender=NamedBoxTemplate)
# allow for externally hosted javascripts (like google code)
models.signals.post_init.connect(utils.swap_storage_engines, sender=Javascript)
models.signals.pre_save.connect(utils.revert_storage_engines, sender=Javascript)
# refresh site cache on commune/section creation/update
models.signals.post_save.connect(utils.refresh_local_site, sender=Commune)
models.signals.post_save.connect(utils.refresh_local_site, sender=Application)
models.signals.post_save.connect(utils.refresh_local_site, sender=Section)
